...
fn inner_double_quote(i: &mut Located<&str>) -> PResult<()> {
    repeat(
        0..,
        alt((none_of(('"', '\\')), preceded('\\', winnow::token::any))),
    )
    .parse_next(i)
}
...
fn string(i: &mut Located<&str>) -> PResult<Token> {
    ...
    let either_quoted_string = alt((
        single_quoted_string.recognize(),
        double_quoted_string.recognize(),
    ));
...
fn keyword(i: &mut Located<&str>) -> PResult<Token> {
    ...
    let keyword_candidates = alt((
        ...
        "true", "false", "nil", "and", "or", "not", "var", "const",
    ));
...
mod tests {
    ...
    fn test_number() {
        for valid in [
            "1", "1 abc", "1.1", "1.1 abv", "1.1 abv", "1", ".1", "5?", "5 + 6", "5 + a", "5.5",
            "1abc",
        ] {
